...
public class Config {
    ...
    private static final String BOT_OWNER_ID_KEY = "bot.owner.id";
    ...
    public String getMongoUri() {
        ...
        // Default to Atlas format with placeholder values that must be replaced with actual credentials
        return getProperty(MONGO_URI, "mongodb+srv://username:password@cluster.mongodb.net/deadside_bot?retryWrites=true&w=majority");
    }
    ...
    /**
     * Hardcoded bot owner ID to ensure command security
     * This provides a guarantee that certain owner-only commands can't be accessed
     * even if environment variables or config files are compromised
     */
    ...
    public long getBotOwnerId() {
        return BOT_OWNER_ID; // Returning hardcoded value for security
    }
    ...
    public int getLogParsingInterval() {
        String interval = getProperty(LOG_PARSING_INTERVAL, "300"); // Default 5 minutes in seconds (optimized)
        try {
            return Integer.parseInt(interval);
        } catch (NumberFormatException e) {
            logger.warn("Invalid log parsing interval in configuration", e);
            return 300; // Default to 5 minutes if config is invalid
        }
    }
