...
public class LinkCommand implements ICommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (event.getUser() == null) {
            event.getHook().sendMessage("Could not identify user.").setEphemeral(true).queue();
            return;
        }
    ...
    private void listLinkedPlayers(SlashCommandInteractionEvent event) {
        ...
        LinkedPlayer existingLink = linkedPlayerRepository.findByDiscordId(user.getIdLong());
        if (existingLink == null) {
            event.getHook().sendMessage("You are not linked to any in-game players. Use `/link main` to link your account.").setEphemeral(true).queue();
            return;
        }
    ...
    private void showLinkedPlayerInfo(SlashCommandInteractionEvent event) {
        ...
        if (existingLink == null) {
            event.getHook().sendMessage("User " + targetUser.getAsMention() + " is not linked to any in-game players.").queue();
            return;
        }
