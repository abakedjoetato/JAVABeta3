    ...
    private int processDeathLog(GameServer server, String content) {
        ...
        for (String line : lines) {
            ...
            try {
                ...
                String killerPlatform = (parts.length > 7) ? parts[7] : "Unknown";
                String victimPlatform = (parts.length > 8) ? parts[8] : "Unknown";
                ...
                try {
                    ...
                    // If this death is older than our last processed timestamp, skip it
                    if (deathTimeMs < server.getLastProcessedTimestamp()) {
                        logger.debug("Skipping old death entry from {} (server last processed: {})", 
                                new java.util.Date(deathTimeMs), 
                                new java.util.Date(server.getLastProcessedTimestamp()));
                        continue;
                    }
                } catch (ParseException e) {
                    logger.warn("Could not parse death timestamp: {}", timestamp, e);
                    continue;
                } catch (Exception e) {
                    logger.warn("Error processing timestamp: {}", timestamp, e);
                    continue;
                }
        ...
        if (count > 0) {
            // Set to current time to avoid reprocessing these entries on the next run
            long currentTime = System.currentTimeMillis();
            server.setLastProcessedTimestamp(currentTime);
            logger.debug("Updated server {} last processed timestamp to {}", server.getName(), currentTime);
        }
